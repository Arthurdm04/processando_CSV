...
# --- 1. Carregamento e Consolidação de Dados (Paralelizado) ---
...
def consolidar_arquivos_csv_paralelo(caminho_fonte: str, caminho_saida_arquivo: str) -> Optional[pd.DataFrame]:
    """
    Lê todos os arquivos CSV que correspondem a 'teste_*.csv' de forma paralela,
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        # map executa a função _ler_csv para cada item na lista arquivos_csv
        resultados = executor.map(_ler_csv, arquivos_csv)
...
# --- 2. Funções Auxiliares para Cálculo de Métricas Genéricas (Sem alterações) ---
...
# --- 3. Funções de Cálculo de Métricas Específicas (Sem alterações) ---
...
# --- 4. Processamento Principal dos Dados (Paralelizado) ---
...
def processar_dados_tribunais_paralelo(df_consolidado: Optional[pd.DataFrame], caminho_saida_arquivo: str) -> Optional[pd.DataFrame]:
    """
    Processa os dados de cada tribunal em paralelo para calcular as métricas
    ...
    todos_resultados = []
    # Cria uma lista de tuplas (sigla, dataframe_do_grupo) para passar para o executor
    grupos_de_tribunais = list(df_consolidado.groupby('sigla_tribunal'))

    # Processa cada tribunal em uma thread separada
    with concurrent.futures.ThreadPoolExecutor() as executor:
        # map executa a função _processar_um_tribunal para cada grupo
        resultados = executor.map(_processar_um_tribunal, grupos_de_tribunais)
        todos_resultados = list(resultados)
...
# --- 5. Geração de Gráficos (Paralelizada) ---

def _gerar_um_grafico(args: Tuple[str, pd.DataFrame, str, int]):
    """Função auxiliar para gerar um único gráfico em uma thread."""
    nome_metrica, df_resumo, caminho_saida, top_n_tribunais = args
...
def gerar_graficos_resumo_paralelo(df_resumo: Optional[pd.DataFrame], caminho_saida: str):
    """
    Gera e salva gráficos de barras comparativos de forma paralela para uma seleção de métricas chave.
    """
    tempo_inicio = time.time()
    ...
    # Prepara os argumentos para cada chamada da função de geração de gráfico
    args_para_graficos = [
        (metrica, df_resumo, caminho_saida, top_n_tribunais) for metrica in metricas_para_plotar
    ]

    # Gera cada gráfico em uma thread separada
    with concurrent.futures.ThreadPoolExecutor() as executor:
        executor.map(_gerar_um_grafico, args_para_graficos)
...
if __name__ == "__main__":
    ...
    print("--- 🚀 Começando o Pipeline de Processamento de Dados (Versão Paralelizada)! ---")
    ...
    # Passo 3: Gerar gráficos visuais em paralelo a partir do resumo de métricas
    if dados_resumo_metricas is not None:
        gerar_graficos_resumo_paralelo(dados_resumo_metricas, DIRETORIO_SAIDA)
    else:
